...
def validate_image_file(value):
    ...
    valid_image_extensions = ['jpg', 'jpeg', 'png'] # Validate extension
    name, ext = os.path.splitext(value.name)  # Extract file extension
    ext = ext[1:].lower()


    if '.' in name :
        raise ValidationError( "Double extension are not allowed")

    if ext not in valid_image_extensions:
        raise ValidationError(f"Unsupported file extension: {ext}. Allowed extensions are: {', '.join(valid_image_extensions)}")
    
    # Validate MIME type using PIL
    try:
        value.seek(0)
        image = Image.open(value)
        image.verify()  # Verify if it's an actual image
    except Exception:
        raise ValidationError("Invalid image file or corrupted file.")
...
def validate_pdf_for_malicious_content(file):
    try:
        ...
        # 1. Check for Embedded JavaScript in PDF Objects**
        for obj in reader.trailer["/Root"].keys():
            if obj in ["/Names", "/OpenAction", "/JavaScript"]:
                raise ValidationError("PDF contains embedded JavaScript, which is not allowed.")

        # 2. Check Each Page for JavaScript or Malicious Content**
        malicious_patterns = [
        ...
        for page_num in range(len(reader.pages)):
            ...
            # Fix: Properly Check for JavaScript in IndirectObjects**
            if isinstance(page, IndirectObject):
                page = page.get_object()  # Convert IndirectObject to a readable dictionary
                    ...
                    for annot in page["/Annots"]:
                        ...
                        if "/A" in annot_obj and "/URI" in annot_obj["/A"]:
                            ...
                            if "javascript:" in uri or "file://" in uri:
                                raise ValidationError(" PDF contains potentially malicious links.")

            # 3. Check Extracted Text for XSS Patterns**
            content = page.extract_text()
            if content:
                for pattern in malicious_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        raise ValidationError(f" PDF contains potentially malicious content: {pattern}")

        # 4. Check for OpenAction (Auto-executing JavaScript)**
        ...
        with Image.open(image) as img:
            ...
            if exif_data:
                for tag, value in exif_data.items():
                    tag_name = ExifTags.TAGS.get(tag, tag)  # Convert numeric ID to readable tag
                    all_metadata[tag_name] = str(value)  # Convert value to string

            # Get extra metadata (like "Comment" field in PNG & JPEG)
            ...
            for key, value in all_metadata.items():
                if any(x in value.lower() for x in ["<script>", "<style>", "javascript:"]):
                    raise ValidationError(f" Image contains suspicious metadata ({key}: {value})")
